...
path = data_dir + "/fig_cover_0517_made_170919.csv" 


# SURPLUS EXAMPLE
curr_mo = "04" # Use two digits
curr_fy =    "17" # Use two digits

prev_mo = "03"  # Use two digits
prev_fy = "16"  # Use two digits

path = data_dir + "/fig_cover_0417_made_170921.csv" 
...
# Receipts: Stage1_Source | Stage1_Amount | Stage2_Receipt_Type | Link
...
df_rec.rename(columns ={'amt': 'Stage1_Amount'}, inplace = True)
...
df_rec.sort_values(by=['Stage1_Amount'], ascending=False, inplace=True)
...
if len(df_rec['Stage1_Source'].str.contains("Deficit").unique().tolist()) > 1:
    df_rec = df_rec[df_rec['Stage1_Amount'] != 0]
...
"""HERE YOU NEED TO ADD A WAY TO REMOVE THE SURPLUS - double check this with an actual surplus month"""
...
df_out['Stage4_Function_Other'] =  np.where(df_out['other_group']=="Yes", "Other", df_out['Stage4_Function'])


# Groupby the new stage4 function with other variable to combine all the other category's values

df_out2 = df_out.groupby(['Stage4_Function_Other'], as_index=False)['Stage3_Outlay_Amount'].sum().reset_index(drop=True)

df_out_other = pd.merge(df_out, df_out2, how='right', on=['Stage4_Function_Other', 'Stage3_Outlay_Amount'])
...
index_other = df_out_other[(df_out_other.loc[:, 'Stage4_Function_Other']=="Other")].index.tolist()[0]

df_out_other['Link'][index_other] = "link"
df_out_other['Stage3_Outlay_Type'][index_other] = "Outlay"
df_out_other['Stage4_Function'][index_other] = "Multiple Functions"
...
### ADD IN SPACERS & FAKE SOURCE BEFORE MERGING WITH MODEL ====================
    ...
    # Fake Source is for Pillar 2

## SPACERS---------------------------------------------------------------------

# Create another dataframe based on (receipts values - 1) then concat with df_receipts and sort
    # Don't create a changed assignment using: df_receipts = df_r_space ...woops
...
## FAKE SOURCE-----------------------------------------------------------------
    # Take the smallest Spacer, which would've been deleted later, and rename to Fake Source
...
# Dummy Source's value is currently $1 less than the smallest source (estate), so it won't add much space
# we need to make this much larger. Let's start with 20% of all outlays/rec+def to see what it looks like
...
dummy_amt = 100000000000 # hard coding the dummy amount for now 
...
### PILLAR 1 CUMULATIVE SUMS (Sources with spacers) ===========================
    ...
    # We don't want to include the deficit or the dummy source in pillar 1
...
size_rec_spacer = 50000000000 # 50B
total_space = size_rec_spacer*num_receipt_spacers
num_outlay_spacers = len(df_outlays) - 1 
size_out_spacer = int(total_space/num_outlay_spacers)
...
"""
total_space = int((deficit+dummy_amt)*1.5) #come back to this and play around with shape (right now about 20/block)
size_rec_spacer = int(total_space/num_receipt_spacers)


# OUTLAYS 
num_outlay_spacers = len(df_outlays) - 1 # different than receipts because of deficit
size_out_spacer = int(total_space/num_outlay_spacers)



if size_out_spacer < 25000000000:
    size_out_spacer = 25000000000
    total_space = 25000000000*num_outlay_spacers
    size_rec_spacer = int(total_space/num_receipt_spacers)
"""
...
for i in range(len(df_r)):
    if df_r['Stage1_Source'][i].startswith("Spacer"):
        ...
        df_r['Stage1_Source_Amount'][i] = df_r['Stage1_Source_Amount'][i]
...
for i in range(4, len(df_r)):
    df_r['Stage2_Bot'][i]=df_r['Stage2_Top'][i-4] # i-4 to skip over the spacers

# The skipping over spacers thing by doing i-4 works everywhere BUT when estate (or lowest source)
    #dummy source and deficit are next to each other without spacers.
...
# Force Dummy Soource's positions to be zero for both pillars, top and bottom 
for i in range(len(df_r)):
...
"""|--------------------------------------------------------------------|"""
"""CHANGE THIS SO IT USES THE MTS VERSION FROM THE FILE NAME ABOVE"""

date = datetime.today().strftime("%y%m%d")
MTS_version = "0517"
path = sankey_dir + "/fig0_cover_" + MTS_version + "_modified_for_sankey_receipts_only_" + str(date) + ".csv"
df_r.to_csv(path, index=False, header=True)
print(path)
...
### PILLAR 3 POSITIONS/CUMSUM (OUTLAYS) ========================================
    ...
    # Don't include spacer values--> separate, manipulate, concat back together
    # For Pillar 3's cumsum, we want real data + dummy - not the spacers.
...
df_dum = df_out.loc[df_out['Stage3_Outlay_Type']=="Dummy"] #just the dummy
...
df_out_nospace = df_out.loc[df_out['Stage4_Row_Type']=="Real"] #just the real 
...
### PILLAR 4 POSITIONS/CUMSUM (FUNCTIONS) =====================================
    # To create Pillar 4's positions, we DO need the spacers, but NOT the dummy function 
...
# HARD CODING THE SPACER SIZE  - TOP HALF SMALL, BOTTOM HALF LARGE
"""
Bottom 4 - 40B spacer (160)
Mid 4 - 25B (100)
Top 4 - 10B spacer (40)
"""
# x is the interval size if i split into three groups, in this case 8 
# so in the first ranked 0-7, then take only the spacers...
x = int(df_out['pillar4_rank_n'].max()/3)
total_space

for i in range(len(df_out)):
    if ((df_out['pillar4_rank_n'][i] < x) & (df_out['Stage4_Function'][i].startswith("Spacer"))):
        df_out['Stage3_Outlay_Amount'][i] = 40000000000
    elif ((df_out['pillar4_rank_n'][i] >= x) & (df_out['pillar4_rank_n'][i] < 2*x) & (df_out['Stage4_Function'][i].startswith("Spacer"))):
        df_out['Stage3_Outlay_Amount'][i] = 25000000000
    elif ((df_out['pillar4_rank_n'][i] >= 2*x) & (df_out['Stage4_Function'][i].startswith("Spacer"))):
        df_out['Stage3_Outlay_Amount'][i] = 10000000000
...
for i in range(len(df_out)):
    if df_out['Stage4_Function'][i].startswith("Spacer"):
        ...
        df_out['Stage3_Outlay_Amount'][i] = df_out['Stage3_Outlay_Amount'][i]
"""
...
# Skipped over ones: 
    #This will get fixed when we address dummy
...
date = datetime.today().strftime("%y%m%d")
MTS_version = "0517"
path = sankey_dir + "/fig0_cover_" + MTS_version + "_modified_for_sankey_outlays_only_" + str(date) + ".csv"
df_out.to_csv(path, index=False, header=True)
