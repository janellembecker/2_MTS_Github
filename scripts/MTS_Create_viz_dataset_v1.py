...
GOALS OF THIS SCRIPT:
    ...
    --Ok, now do this for all files in the folder?
...
x.rename(columns ={'recpt': 'amount'}, inplace = True)
y.rename(columns ={'neg_outlays': 'amount'}, inplace = True)
z.rename(columns ={'deficit': 'amount'}, inplace = True)

x.rename(columns ={'amount': 'amount_RO'}, inplace = True)
y.rename(columns ={'amount': 'amount_RO'}, inplace = True)
z.rename(columns ={'amount': 'amount_DS'}, inplace = True)
...
# <><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>#
"""|--------------------------------------------------------------------|"""
"""|--STEP 4: MAKE df's TO DRIVE THE VISUALIZATIONS: Table 7-->Fig 3    |"""
"""|--------------------------------------------------------------------|"""

# Figure 3 shows Receipts over time by Source
    #This requires Table 7 ==> read in the data and wrangle!

"""|--------------------------------------------------------------------|"""
"""| READ IN THE DATA - Table 7 (R/O By Month)                          |"""
"""|--------------------------------------------------------------------|"""
# Table 9 gives Source and Function for Receipts/Outlays

whatiwant = {col: str for col in (0,14)}
...
"""|--------------------------------------------------------------------|"""
"""| WRANGLE THE DATA - Table 7 (R/O By Month)                          |"""
"""|--------------------------------------------------------------------|"""

# Remove whitespace in column names -------------------------------------------
df7.columns.tolist() #oh it's aa million new lines

    #Rename columns
cols_df7 = df7.columns.tolist()
rename_columns = ['R_source_O_agency',
                 'oct',
                 'nov',
                 'dec',
                 'jan',
                 ...
                 'fytd',
                 'comp_per_pfy']
for (oldcolname, replacement) in zip(cols_df7, rename_columns):
     df7.rename(columns={oldcolname : replacement}, inplace=True)
df7.columns.tolist()    #check that it went right

### Add in year since it wasn't a part of this table anywhere but the title
df7['fy'] = ""
df7['fy'] = "20" + str(path[74:76])
...
df7['totals'] = False
    ...
    # Outlays index value
bool_vector = df7.loc[:,'R_source_O_agency'] == "Outlays:"
index_out = df7[bool_vector].index.tolist()
index_out = index_out[0] # 17, so 0-16 is receipts

    # Totals index value
bool_vector = df7.loc[:,'R_source_O_agency'] == "   Totals This Year:"
index_tot = df7[bool_vector].index.tolist()
index_tot = index_tot[0] #98, so 17-97 is outlays, and 98-end is totals
...
for i in range(index_out, index_tot):
    df7['outlay'][i] = True

for i in range(index_tot, len(df7)):
    df7['totals'][i] = True


### Create dataframe for the figure -------------------------------------------

df_fig3 = df7[(df7['rec'] != False)] #receipts only now by source each month


### Keep data wrangling -------------------------------------------------------

#Drop "receipt" row
df_fig3.drop(df_fig3.index[0], inplace=True)

#Rename column 
df_fig3.rename(columns ={'R_source_O_agency': 'source'}, inplace = True)


# Strip whitespace from source_func column 
df_fig3['source'] = df_fig3['source'].str.strip()
df_fig3['source'] = df_fig3['source'].astype(str)

# Make new column based on old column 
df_fig3['source_parent'] = df_fig3['source']
...
# Rename those cells
df_fig3['source_parent'][index_EGR] = "Social Insurance and Retirement Receipts"
df_fig3['source_parent'][index_UI] = "Social Insurance and Retirement Receipts"
df_fig3['source_parent'][index_OR] = "Social Insurance and Retirement Receipts"

df_fig3.drop(df_fig3.index[index_SIRR], inplace=True)
df_fig3.reset_index(drop=True, inplace=True)

### Convert numbers to integers -----------------------------------------------

col_df_fig3 = df_fig3.columns.tolist()
...
# remove all '......' serving as "no transactions"
df_fig3 = df_fig3.replace('......', '0')

# remove all commas, convert to numbers 
for name in col_list:
    df_fig3[name] = df_fig3[name].astype(str)
    df_fig3[name] = df_fig3[name].str.replace(',', '')
    df_fig3[name] = df_fig3[name].astype(float)




###############################################################################
#            Iterating in order to create stuff in Tableau    (Fig 3)         #
###############################################################################
...
# df_fig3 is currently clean and wide. Tableau needs LONG 

#Drpo columns not needed
df_fig3.columns.tolist()



df_fig3 = df_fig3[['source',
                    'source_parent',
                    'oct',
                    'nov',
                    'dec',
...
list_of_months = ['nov','dec','jan','feb','mar','apr','may',
                'jun','jul','aug','sept']


october = df_fig3[['source',
                    'source_parent',
                    'oct',
                    'fy']]

october['month']= "oct"
october.rename(columns= {"oct" : "amount"}, inplace=True)
...
for month in list_of_months:
    ...
    x  = df_fig3[['source',
                  ...
                  'fy']]
    x['month'] = str(month)
    x.rename(columns= {str(month) : "amount"}, inplace=True)
    master = pd.concat([master, x], axis=0, ignore_index=True)
...
"""|--------------------------------------------------------------------|"""
"""|--STEP 5: MAKE df's TO DRIVE THE VISUALIZATIONS: ???? --> Fig 4     |"""
"""|--------------------------------------------------------------------|"""
